...
$app->post('/orders', function () use($app){
  ...
  try{
    ...
    foreach ($json_data->menus as $key => $order_item) {
      ...
      R::exec(
        ...
        array(
          ...
          date("Y-m-d H:i:s"), $json_data->table->company_id, $json_data->table->branch_id, $order_item->amount, '',
          0, 0, $order_item->amount, '', $order_item->tambahan == true ? 1 : 0,
    ...
    R::exec(
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      array(
        date("Y-m-d H:i:s"), $order_id, "evan", $json_data->note == null ? '' : $json_data->note , 0,
